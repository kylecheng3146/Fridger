...
class RefreshTokenRepositoryTest {
    ...
    @Test
    fun `store should insert a new token record`() = runTest {
    ...
    @Test
    fun `findValid should return null for non-existent token`() = runTest {
        assertNull(refreshTokenRepository.findValid("non-existent-hash"))
    }

    @Test
    fun `findValid should return null for expired token`() = runTest {
    ...
    @Test
    fun `rotate should fail if old token does not exist`() = runTest {
    ...
    @Test
    fun `rotate should create new token and delete old one`() = runTest {
    ...
    @Test
    fun `revokeByHash should delete the token`() = runTest {
        val expiresAt = Instant.now().plus(Duration.ofDays(30))
    ...
    @Test
    fun `revokeByHash should return false for non-existent token`() = runTest {
